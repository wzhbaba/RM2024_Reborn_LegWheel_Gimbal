...
class Referee
{
   public:
    ...
    void KeyProcess();
    uint8_t GetKeyPress(int16_t _key)
    {
        return keyboard_[_key].is_pressed;
    };
    uint8_t GetKeyTick(int16_t _key)
    {
        return keyboard_[_key].is_ticked;
    };

    void SetKeyTick(int16_t _key, uint8_t _tick)
    {
        keyboard_[_key].is_ticked = _tick;
    };
